  ...
  // Validate string as email using validator.js
  async function isEmail(str){
    const { default: validator } = await validatorLib;
  ...
  function renderLogin(){
    ...
    if(hasSession){
      ...
      if(list.length > 1 && chips && chipsRow){
        ...
        chips.innerHTML = list.map(a=>`<span class="acct-chip" data-email="${esc(a.email)}">${a.avatar?`<img src="${esc(a.avatar)}" alt="">`:'<i class="bi bi-person"></i>'}<span class="small">${esc(a.email)}</span></span>`).join('');
    ...
    if(form){
      form.onsubmit = async (e)=>{
        ...
        if (window.jQuery && $.fn && $.fn.validate) { if(!$('#loginForm').valid()){ toast('Please fix the highlighted fields'); return; } }
          ...
          if(!hasSession && !(await isEmail(emailTest))){ toast('Enter a valid email'); return; }
        }
  ...
  function renderRegister(){
    ...
    if(form){ form.onsubmit = async (e)=>{
      ...
      if (window.jQuery && $.fn && $.fn.validate) { if(!$('#registerForm').valid()){ toast('Please fix the highlighted fields'); return; } }
        ...
        if(!(await isEmail(emailTest))){ toast('Please enter a valid email'); return; }
      }
      ...
      axios.post(`${API_URL}/accounts`, { email, name }).catch(()=>{});
  ...
  function renderForgot(){
    ...
    const form = $qs('#forgotForm'); if(form){ form.onsubmit = async (e)=>{ e.preventDefault(); if (window.jQuery && $.fn && $.fn.validate) { if(!$('#forgotForm').valid()){ toast('Enter a valid email'); return; } } else { if(!form.checkValidity()){ form.classList.add('was-validated'); toast('Enter a valid email'); return; } const emailTest = $qs('#fpEmail')?.value?.trim()||''; if(!(await isEmail(emailTest))){ toast('Enter a valid email'); return; } } toast('Reset link sent'); route('/login'); }; }
  }
  ...
  async function boot(){
    ...
    if(yearEl){ const { default: dayjs } = await import('https://cdn.jsdelivr.net/npm/dayjs@1/dayjs.min.js'); yearEl.textContent = dayjs().year(); }
