...
write_env_file() {
  ...
  python3 - "$env_path" "$port" "$public_port" "$force_tls" "$public_base" \
    "$domain" "$fallback_host" "$origins" "$cert_path" "$key_path" "$le_email" <<'PY'
import sys
...
values = {
    ...
    "FORCE_TLS": sys.argv[4],
    "PUBLIC_BASE_URL": sys.argv[5],
    "PUBLIC_DOMAIN": sys.argv[6],
    "PUBLIC_FALLBACK_HOST": sys.argv[7],
    "ALLOWED_ORIGINS": sys.argv[8],
    "PRODUCT_BACKUPS": "3",
    "TRUST_PROXY_HEADERS": "1",
    "TLS_CERT_FILE": sys.argv[9] or None,
    "TLS_KEY_FILE": sys.argv[10] or None,
    "LETS_ENCRYPT_EMAIL": sys.argv[11] or None,
}
...
with env_path.open("w", encoding="utf-8") as fh:
    fh.write(text)
PY
  echo "Updated admin/.env with selected options."
}
